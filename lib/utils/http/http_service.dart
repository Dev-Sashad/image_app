...
import 'package:image_app/utils/http/response_handler.dart';
...
class HttpService {
  ...
  Future<dynamic> post(String path, Map<dynamic, dynamic> body) async {
    ...
    final response = await http
        .post(Uri.parse(url), headers: await getHeaders(), body: json.encode(body))
  ...
  Future<dynamic> patch(String path, Map<dynamic, dynamic> body) async {
    ...
    final response = await http
        .patch(Uri.parse(url), headers: await getHeaders(), body: json.encode(body))
        .timeout(const Duration(seconds: 30));
